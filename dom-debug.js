 ...
 * JavaScript version - runs directly without compilation
 */
 ...
 */
async function inspectDOM(page, selector) {
  try {
 ...
 */
async function screenshot(page, testName, issue) {
 ...
 */
function recordIssue(test, selector, issue, found = false) {
  const entry = { test, selector, issue, found };
...
async function testLogin(page) {
  ...
  try {
    ...
    log('Navigating to login page (may take 2-5 minutes)...', 'cyan');
    await page.goto('https://stations.epump.africa/login', { waitUntil: 'domcontentloaded', timeout: 120000 }).catch((e) => {
      log(`⚠️ Navigation timeout: ${String(e).slice(0, 50)}`, 'yellow');
    });
    
    log('Waiting for network to settle...', 'cyan');
    await page.waitForLoadState('networkidle', { timeout: 60000 }).catch(() => {
      log('⚠️ Network idle timeout (continuing anyway)', 'yellow');
    });
    ...
    // Perform login
    if (emailInfo.exists) {
      log('Filling login credentials...', 'cyan');
      ...
      log('Waiting for page response...', 'cyan');
      // Wait for navigation with long timeout
      await page.waitForLoadState('networkidle', { timeout: 60000 }).catch(() => {
        log('⚠️ Network idle timeout (continuing)', 'yellow');
      });
      
      await page.waitForTimeout(2000); // Extra wait for DOM to settle
 ...
 */
async function testPriceLog(page) {
  ...
  try {
    ...
    if (!priceLogLink) {
      ...
      log('Trying Price Management link...', 'cyan');
...
async function runDOMDebugTests() {
  ...
  try {
    ...
    // Set much longer timeouts for slow network
